...
for i in range(x.shape[0]):
    ...
    Xte = data_test[:, 0:data_train.shape[1] - 1]
    ...
    for q in range(c):
        if Ypr[q] == Yte[q]:
            num += 1
    g=num * 1.0 / c
    # print('HOG+SIFT-ACC: %.3f' % (num * 1.0 / c))
    ...
    for j in range(d):
        if Ypr[j] == Yte[j]:
            num += 1
    # print('SIFT+HOG using lowess-ACC: %.3f' % (num * 1.0 / d))
    y.append(num * 1.0 / d)
yq=np.array(y)
plt.scatter(x, yq,marker=".")
plt.show()
ymaxloc=np.argmax(yq)
print('Only SIFT+HOG-ACC: %.3f' % (g))
# print(ymaxloc)
print('SIFT+HOG using lowess-ACC: %.3f' % yq.max())
print('finsh')
